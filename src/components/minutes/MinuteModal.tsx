...
export default function MinuteModal({
    ...
    useEffect(() => {
        if (isOpen) {
            if (minuteToEdit) {
                setFormData({
                    meetingDate: minuteToEdit.meeting_date,
                    location: minuteToEdit.location || '',
                    startTime: minuteToEdit.start_time,
                    endTime: minuteToEdit.end_time,
                    notes: minuteToEdit.notes || '',
                    status: minuteToEdit.status,
                });
            } else {
                setFormData({
                    meetingDate: new Date().toISOString().split('T')[0],
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-200">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900">
                            {minuteToEdit ? 'Editar Acta' : 'Nueva Acta'}
                        </h2>
                        {projectCode && (
                            <p className="text-sm text-gray-600 mt-1">Proyecto: {projectCode}</p>
                        )}
                        {minuteToEdit && (
                            <p className="text-sm text-gray-600 mt-1">Acta #{minuteToEdit.minute_number}</p>
                        )}
                    </div>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div className="flex gap-3 pt-4">
                        <Button
                            ...
                            className="flex-1"
                        >
                            {loading ? 'Guardando...' : (minuteToEdit ? 'Guardar Cambios' : 'Crear Acta')}
                        </Button>
                        <Button
                            type="button"
                            variant="outline"
